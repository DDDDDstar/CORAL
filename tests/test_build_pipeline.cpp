...
int main(int argc, char** argv) {
    std::string base_data_file, query_test_file, gt_test_file;
    ...
    fpath log_file, graph_file, cache_file, recall_file, query_file, config_file, train_gt_file;
    ...
    try {
        ...
        desc.add_options()("N", po::value<size_t>(&N)->required()->default_value(0), "base n");
        ...
        desc.add_options()("train_gt_file",
                           po::value<fpath>(&train_gt_file)->default_value(fpath("")),
                           "train_gt_file in bin format");
        ...
        desc.add_options()("query_test_file",
                           po::value<std::string>(&query_test_file)->default_value("None"),
                           "query_test_file");
        desc.add_options()("gt_test_file",
                           po::value<std::string>(&gt_test_file)->default_value("None"),
                           "gt_test_file");
        ...
        desc.add_options()("k", po::value<int>(&k)->default_value(1)->required(),
                           "k nearest neighbors");
        desc.add_options()("iso_thres", po::value<float>(&iso_thres)->default_value(0.9),
        ...
        desc.add_options()("log_file", po::value<fpath>(&log_file)->default_value(""),
                           "log file path");
        desc.add_options()("config_file", po::value<fpath>(&config_file)->default_value("default"),
                           "config file path");
    ...
    // cudaMallocHost(&data_bp, sizeof(float) * base_num * base_dim);
    // efanna2e::load_data_host<float>(base_data_file.c_str(), base_num, base_dim, data_bp);
    ...
    if (dist_metric == efanna2e::INNER_PRODUCT) {
        normalize_L2_omp(aligned_data_bp, base_dim, base_dim);
        normalize_L2_omp(aligned_data_sq, sq_num, sq_dim);
    }
    std::string date_time = efanna2e::getCurrentDateTimeString(), version;
#ifdef algo0
    version = "v0";
#endif
#ifdef algo1
    ...
    const std::string recall_thres_str = std::to_string(int(recall_thres)),
                      ...
                      dataset_dist = dataset + "_" + dist + "_" + TOS(M_pjbp);
    fpath gt_dir = fpath(GT_PREFIX) / dataset_dist;
    graph_file =
        fpath(INDEX_PREFIX) / (dataset_dist + "_" + query_thres_str + "_" + TOS(CE ? 1 : 0));
    if (log_file.string() == "")
        log_file = fpath(LOG_PREFIX) / "build" / (dataset_dist + "_" + query_thres_str + ".log");
    else
        log_file = fpath(LOG_PREFIX) / log_file;
    recall_file = fpath(RECALL_PREFIX) / (dataset_dist + ".csv");
    cache_file = gt_dir / "knn.cache";
    query_file = gt_dir / "query.cache";
    efanna2e::fo.print(
        "Index graph save path: " + graph_file.string() + "\nlog path: " + log_file.string() +
        "\nrecall path: " + recall_file.string() + "\nknn cache path: " + cache_file.string());
    ...
    parameters.Set<int>("max_degree", M_pjbp);
    parameters.Set<int>("L_pjpq", L_pjpq);
    parameters.Set<int>("num_threads", num_threads);
    parameters.Set<int>("CE", CE);
    parameters.Set<int>("k", k);
    parameters.Set<std::string>("cache_file", cache_file.string());
    parameters.Set<std::string>("base_file", base_data_file);
    parameters.Set<std::string>("query_test_file", query_test_file);
    parameters.Set<std::string>("gt_test_file", gt_test_file);
    parameters.Set<std::string>("graph_file", graph_file.string());
    parameters.Set<std::string>("query_file", query_file.string());
    parameters.Set<std::string>("train_gt_file",
                                !train_gt_file.empty() ? train_gt_file.string() : "None");
    parameters.Set<std::string>("log_file", log_file.string());
    parameters.Set<std::string>("recall_file", recall_file.string());
    parameters.Set<std::string>("config_file", config_file.string());
    ...
    fo.print("omp_set_num_threads: " + TOS(num_threads));
    omp_set_num_threads(num_threads);
    auto s = std::chrono::high_resolution_clock::now();
    base_num = N > 0 ? N : base_num;
    ...
    efanna2e::fo.print("Save index to " + graph_file.string());
