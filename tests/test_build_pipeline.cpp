...
int main(int argc, char **argv) {
    ...
    try {
        ...
        desc.add_options()("CE", po::value<uint32_t>(&CE)->default_value(1),
                           "Use connectivity enhancement or not");
        desc.add_options()("k", po::value<uint32_t>(&k)->default_value(1)->required(), 
                           "k nearest neighbors");
    ...
    efanna2e::IndexPipeline index_pipeline(base_dim, base_num + sq_num, dist_metric, nullptr);
    ...
    index_pipeline.BuildPipeline(sq_num, aligned_data_sq, base_num, aligned_data_bp, parameters);
