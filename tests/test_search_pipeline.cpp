...
int main(int argc, char **argv) {
    ...
    try {
        ...
        desc.add_options()("L_pq", po::value<std::vector<uint32_t>>(&L_vec)->multitoken(),
    ...
    efanna2e::IndexPipeline index(q_dim, base_num + sq_num, dist_metric, nullptr);
    ...
    for (uint32_t i = 0; i < L_vec.size(); ++i) {
        uint32_t L_pq = L_vec[i];
        if (k > L_pq) {
