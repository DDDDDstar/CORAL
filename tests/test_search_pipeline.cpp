...
double ComputeRderr(float *gt_dist, int gt_dim, std::vector<std::vector<float>> &res_dists, int k, efanna2e::Metric metric)
{
    double total_err = 0;
...
int main(int argc, char **argv)
{
    ...
    try
    {
        ...
        desc.add_options()("max_hop", po::value<double>(&max_hop)->default_value(5000),
                           "Max limited hops");
        desc.add_options()("k", po::value<int>(&k)->default_value(1)->required(), "k nearest neighbors");
        desc.add_options()("evaluation_save_path", po::value<std::string>(&evaluation_save_path),
                           "Path prefix for saving evaluation results");
        desc.add_options()("num_threads,T", po::value<int>(&num_threads)->default_value(omp_get_num_procs()),
    ...
    int base_num, base_dim, sq_num = 0, sq_dim = 0;
    ...
    efanna2e::load_gt_data_with_dist<int, float>(gt_file.c_str(), gt_pts, gt_dim, gt_ids, gt_dists);
    ...
    index.LoadGraph(projection_index_save_file.c_str());
    ...
    int *res = new int[q_pts * k];
    memset(res, 0, sizeof(int) * q_pts * k);
    std::vector<std::vector<float>> res_dists(q_pts, std::vector<float>(k, 0.0));
    int *projection_cmps_vec = (int *)aligned_alloc(4, sizeof(int) * q_pts);
    memset(projection_cmps_vec, 0, sizeof(int) * q_pts);
    int *hops_vec = (int *)aligned_alloc(4, sizeof(int) * q_pts);
    ...
    for (int i = 0; i < L_vec.size(); ++i)
    {
        ...
        for (size_t i = 0; i < 100; ++i)
        {
            ...
            index.SearchPipe(aligned_query_data + i * q_dim, k, i, parameters, res + i * k, res_dists[i]);
        }
...
#pragma omp parallel for schedule(dynamic, 1)
        ...
        {
            auto ret_val = index.SearchPipe(aligned_query_data + i * q_dim, k, i, parameters, res + i * k, res_dists[i]);
        ...
        {
            L_vec.push_back(200 + L_vec[L_vec.size() - 1]);
        }
        if (i > 50 || avg_hops > max_hop)
        {
            std::cout << "Queue is too long, judged as unable to meet the recall requirement, termination" << std::endl;
            break;
        }
    }
    if (evaluation_out.is_open())
