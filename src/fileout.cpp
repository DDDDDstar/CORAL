...
void FileOut::Init(std::string filename) {
    ...
    file_.open(filename, std::ios::out | std::ios::app);  // 以追加模式打开文件
    if (!file_) {                                         // 检查文件是否成功打开
...
void FileOut::eprint(std::string msg) {
    std::string decorated_msg = "--------------------ERROR--------------------\n" + msg + "\n" +
                                "---------------------------------------------";
...
void FileOut::qprint(const int head, const int tail, const int len, const int r, const int w) {
    assert(r < len && w < len);
