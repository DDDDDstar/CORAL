...
namespace efanna2e {
// -------------------- 工具函数 --------------------
static inline bool file_exists(const fpath& path) { return std::filesystem::exists(path); }

static inline uint64_t get_file_size(const fpath& path) {
    struct stat st;
    // if (!file_exists(path)) return 0;
    return (uint64_t)std::filesystem::file_size(path);
}

// 从 Slot 构造原始序列化 bytes（未压缩）
// 序列化格式： [deg (4B)] [nbrs (deg * 4B)] [dists (deg * 4B)]
static std::vector<char> serialize_slot_uncompressed(const Slot& s) {
...
// 反序列化
static void deserialize_slot_uncompressed(const char* buf, Slot& s) {
    int deg = 0;
    memcpy(&deg, buf, 4);
    s.deg = deg;
...
void DiskStorage::load_node_slot(int node_id, Slot& slot_out) {
    assert(use);
...
void DiskStorage::load_node_vec(int node_id, float* vec_out) {  // 读取向量（fixed-size）
    assert(use);
    DiskPartition* part = get_partition(node_id);
...
void DiskStorage::write_node_append(int node_id, const Slot& slot, bool want_flush) {
    DiskPartition* part = get_partition(node_id);
    if (!part) fo.eprint("write_node_append: no partition for node " + TOS(node_id));
    part->write_node_append(node_id, slot, want_flush);
    if (!update_flag.load()) update_flag.store(true, std::memory_order_release);
}
...
DiskStorage::DiskStorage(const fpath& base_file, const fpath& graph_file, int node_num, int dim,
    ...
    if (base_file.string().size()) {
        fo.print("open vector file: " + base_file.string());
        vec_fd = open(base_vec_file.c_str(), O_RDONLY);
        if (vec_fd < 0) fo.eprint("DiskStorage::load_node_vec: open vectors for read");
    }
    // 启动 checkpoint 线程（定期持久化 index）
    if (node_num > CPU_N) checkpoint_thread = std::thread(&DiskStorage::checkpoint_loop, this);
...
DiskStorage::~DiskStorage() {
    ...
    for (DiskPartition* p : partitions) delete p;  // partitions will be destroyed automatically
...
void DiskPartition::persist_index(std::vector<NodeDiskIndex>& index_table) {
    // fo.print("persist index with partition " + TOS(partition_id));
    std::unique_lock<std::shared_mutex> lk(write_mutex);
    // std::unique_lock<std::shared_mutex> lk(part_mutex);
    // 以临时文件写入，再替换，保证原子性
    fpath tmp = idx_path;
    tmp += ".tmp";
    int fd = open(tmp.string().c_str(), O_RDWR | O_CREAT | O_TRUNC, 0644);
    // 写索引表
    // size_t need = index_table.size() * sizeof(NodeDiskIndex);
    write(fd, index_table.data(), index_table.size() * sizeof(NodeDiskIndex));
    fsync(fd);
    close(fd);
    // 原子替换
    rename(tmp.string().c_str(), idx_path.c_str());
}

// 读取索引文件到内存
void DiskPartition::load_index(std::vector<NodeDiskIndex>& index_table) {
    int fd = open(idx_path.c_str(), O_RDONLY);
    index_table.resize(node_count);
    ssize_t r = read(fd, index_table.data(), get_file_size(idx_path));
    (void)r;
    close(fd);
}
...
void DiskPartition::compact_once() {
    fo.print("compact with partition " + TOS(partition_id));
    // std::unique_lock<std::shared_mutex> lk(part_mutex);
    PartitionView *old_view = active_view.load(std::memory_order_acquire),
                  *new_view = new PartitionView;
    // 创建临时 new data 文件
    fpath new_data = data_path;
    new_data += ".compact";
    int new_fd = open(new_data.c_str(), O_RDWR | O_CREAT | O_TRUNC, 0644);
    if (new_fd < 0)
        fo.eprint("compact: open new data file error: " + std::string(strerror(errno)));
    // 打开新 append 文件
    fpath new_append = append_path;
    new_append += ".new";
    int new_append_fd = open(new_append.c_str(), O_RDWR | O_CREAT | O_TRUNC, 0644);
    if (new_append_fd < 0)
        fo.eprint("compact: open new append file error: " + std::string(strerror(errno)));
    ...
    // 遍历当前 index，读取每个条目并写入 new_data（如果有数据）
    {
        ...
        for (int i = 0; i < node_count; ++i) {
            const NodeDiskIndex& e = old_view->index[i];
            if (e.size == 0) {
                new_index[i] = {0, 0, DataPos::BASE};
                continue;
            }
            // 读取原数据
            std::vector<char> comp(e.size);
            ssize_t r = pread(e.pos == DataPos::BASE ? old_view->data_fd : old_view->append_fd,
                              comp.data(), e.size, e.offset);
            if (r < 0)
                fo.eprint(std::string(e.pos == DataPos::BASE ? "base" : "append") +
                          " pread error: " + strerror(errno));
            else if (r != (ssize_t)e.size)  // 数据丢失或索引指向无效块
                fo.eprint("compact: missing " +
                          std::string(e.pos == DataPos::BASE ? "base" : "append") +
                          " block for local " + TOS(i) + ": read size-" + TOS(r) +
                          " != expected size-" + TOS(e.size));

            // 写入 new_fd
            ssize_t w = pwrite(new_fd, comp.data(), e.size, write_off);
            if (w != (ssize_t)e.size) {
                close(new_fd);
                unlink(new_data.c_str());
                fo.eprint("compact write error");
            }
            new_index[i] = {write_off, e.size, DataPos::BASE};
            write_off += e.size;
        }
        fsync(new_fd);

        // 构建新视图
        new_view->data_fd = new_fd;
        new_view->append_fd = new_append_fd;
        new_view->index = std::move(new_index);

        rename(new_data.c_str(), data_path.c_str());  // 原子替换文件名
        rename(new_append.c_str(), append_path.c_str());
        append_tail.store(0, std::memory_order_release);
        active_view.store(new_view, std::memory_order_release);
    }
    std::thread([old_view, this]() {  // 延迟销毁旧视图（安全）
        std::this_thread::sleep_for(std::chrono::seconds(2));
        close(old_view->data_fd);
        close(old_view->append_fd);
        delete old_view;
    })
        .detach();
    fo.print("Partition " + TOS(partition_id) + " RCU compaction done: data_size=" +
             TOS(get_file_size(data_path)) + ", write time=" + TOS(write_time));
}
...
void DiskPartition::compaction_loop() {
    while (!stop_background) {
        ...
        uint64_t append_sz = get_file_size(append_path);
        if (append_sz > (uint64_t)512 * 1024 * 1024)  // 512MB
            try {
                compact_once();
            } catch (...) {
...
DiskPartition::DiskPartition(const fpath& base_path, int pid, int partition_start_id,
    ...
    : base_path(base_path),
      ...
      construct(construct) {
    // 文件路径
    data_path = idx_path = append_path = base_path / ("graph.part" + TOS(pid));
    data_path += ".data";
    idx_path += ".idx";
    append_path += ".append";
    fo.print("Partition " + TOS(partition_id) + ":data_path(" + data_path.string() +
             "), idx_path(" + idx_path.string() + "), append_path(" + append_path.string());

    // 打开或创建索引（node_count entries）
    std::vector<NodeDiskIndex> index_table;
    if (construct) {
        index_table.assign(node_count, NodeDiskIndex{0, 0, DataPos::BASE});  // 初始化空索引
        persist_index(index_table);  // 写回索引文件（一次性 allocate）
    } else {
        load_index(index_table);
    }

    // 打开数据文件以供 append / read
    int data_fd = open(data_path.c_str(), O_RDWR | O_CREAT, 0644);
    int append_fd = open(append_path.c_str(), O_RDWR | O_CREAT, 0644);
    if (data_fd < 0) fo.eprint("open data_fd failed");
    if (append_fd < 0) fo.eprint("open append_fd failed");
    // 初始化 append_tail（关键）
    append_tail.store(get_file_size(append_path), std::memory_order_relaxed);

    PartitionView* view = new PartitionView;
    view->data_fd = data_fd;
    view->append_fd = append_fd;
    view->index = index_table;  // 拷贝
    active_view.store(view, std::memory_order_release);
...
DiskPartition::~DiskPartition() {
    fo.print("Partition " + TOS(partition_id) + " destructor called");
    stop_background = true;  // 停止后台线程

    if (compaction_thread.joinable()) compaction_thread.join();

    if (get_file_size(append_path) > 0) compact_once();  // 最后一次 compact

    PartitionView* v = active_view.load();
    persist_index(v->index);  // persist 索引
...
void DiskPartition::load_node(int global_node_id, Slot& slot_out) {
    PartitionView* view = active_view.load(std::memory_order_acquire);
    // std::shared_lock<std::shared_mutex> lk(part_mutex);
    if (!node_in_partition(global_node_id)) fo.eprint("Node not in partition");
    int local_id = global_node_id - start_id;
    NodeDiskIndex& e = view->index[local_id];
    ...
    // posix pread 读取压缩块
    std::vector<char> comp(e.size);
    ssize_t r = pread(e.pos == DataPos::BASE ? view->data_fd : view->append_fd, comp.data(),
                      e.size, e.offset);
    ...
    if (comp_len > 0) {
        int dec = LZ4_decompress_safe(comp_ptr, uncompressed.data(), comp_len, uncompressed_size);
        if (dec < 0) fo.eprint("LZ4 decompress failed node " + TOS(global_node_id));
    } else  // 未压缩：直接拷贝
...
// 如果 want_flush==true 会调用 fsync 来保证持久化（开销高）
void DiskPartition::write_node_append(int global_node_id, const Slot& slot, bool want_flush) {
    // std::unique_lock<std::shared_mutex> lk(part_mutex);
    if (!node_in_partition(global_node_id))
        fo.eprint("DiskPartition::write_node_append: Node not in partition");

    const int local_id = global_node_id - start_id;
    ...
    int maxComp = LZ4_compressBound(raw_sz);
    ...
    if (csz <= 0) {  // 压缩失败，退回不压缩写法
        ...
        comp.resize(csz + 8);
    {
        std::shared_lock<std::shared_mutex> lk(write_mutex);
        PartitionView* view = active_view.load(std::memory_order_acquire);
        // 分配 append offset（顺序追加）
        const uint32_t comp_size = (uint32_t)comp.size();
        const uint64_t off = append_tail.fetch_add(comp_size);
        // 并发安全写
        const ssize_t w = pwrite(view->append_fd, comp.data(), comp_size, off);
        if (w != (ssize_t)comp_size) fo.eprint("write_node_append: pwrite failed");
        ...
        view->index[local_id] = {off, comp_size, DataPos::APPEND};
        if (want_flush) fsync(view->append_fd);
    }
    write_time.fetch_add(1);
}
...
// 回调签名： void callback(bool success, vector<Slot> slots, vector<float> vecs)
...
//     while (true) {
//         std::pair<std::vector<int>, std::function<void(bool, std::vector<Slot>)>> job;
//         {
//             std::unique_lock<std::mutex> lk(prefetch_slot_mutex);
...
//         auto cb = job.second;
//         std::vector<Slot> slots(node_ids.size());
//         bool ok = true;
//         for (size_t i = 0; i < node_ids.size(); ++i)
