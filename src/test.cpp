...
void Test::Run() {
    ...
    //             if (fabs(dist - dists[batch_i * base_n + i]) > 1e-6 || idxs[batch_i * base_n +
    //             i] != i)
    //                 fo.eprint("compute_knn_dist_test ERROR with batch_i: " +
    //                 std::to_string(batch_i) +
    //                           "\ngt(idx, dist): (" + std::to_string(i) + ", " +
    //                           std::to_string(dist) +
    ...
    //                 metric == DIST_METRIC::L2 ? compute_dist_l2(query + batch_i * dim, base + i
    //                 * dim)
    //                                           : compute_dist_ip(query + batch_i * dim, base + i
    //                                           * dim);
    ...
    //                 const CN& res = cand_nbrs_res[batch_i * k * cand_size + pivot_i * cand_size
    //                 + i]; if (i >= k && (res.id != -1 || res.status != Discarded))
    //                     fo.eprint("cand_compute_test ERROR with batch_i: " +
    //                     std::to_string(batch_i) +
    //                               ", pivot_i: " + std::to_string(pivot_i) +
    //                               ", i: " + std::to_string(i) +
    //                               "\ngt(id, dist, idx, status): " + std::to_string(-1) + ", _,
    //                               _, " + std::to_string(Discarded) + "\nVS\nres: " +
    ...
    //                                      : (metric == DIST_METRIC::L2 ? L2_FARTHEST :
    //                                      IP_FARTHEST));

    //                     if (fabs(res.dist - dist) > 1e-6 || res.id != id || res.idx != i ||
    //                         res.status != Initial)
    //                         fo.eprint(
    //                             "cand_compute_test ERROR with batch_i: " +
    //                             std::to_string(batch_i) +
    //                             ", pivot_i: " + std::to_string(pivot_i) + ", i: " +
    //                             std::to_string(i) +
    ...
    //                           return metric == DIST_METRIC::L2 ? a.dist < b.dist : a.dist >
    //                           b.dist;
    //                       });
    ...
    //                 const CN& cand_nbr = cand_nbrs_res[batch_i * k * cand_size + i * cand_size +
    //                 j]; if (cand_nbr.id == -1 || cand_nbr.status == Repeated) continue; bool
    //                 ignore = false;
    ...
    //                     if (metric == DIST_METRIC::L2 ? cand_nbr.dist > dist : cand_nbr.dist <
    //                     dist) {
    ...
    //                 if (id == -1)
    //                     fo.eprint("cand_ignore_test ERROR: id == -1 with pivot_id: " +
    //                     TOS(pivot_id));
    ...
    //                     fo.eprint("cand_ignore_test ERROR with batch_i: " +
    //                     std::to_string(batch_i) +
    //                               ", i: " + std::to_string(i) + ", j: " + std::to_string(j) +
    //                               "\ngts(" + std::to_string(new_nbr_ids_gt.size()) + "): " + gts
    //                               +
    //                               "\nVS\nres(" + std::to_string(j + 1) + "): " + ress);
    //                 new_nbr_num--;
    //             }
    // #else
