...
uint32_t *GPUFuncs::handle_knn_updates(const uint32_t *d_knn_idxs, const int batch)
{
    ...
    cub::DeviceSegmentedRadixSort::SortPairs(
        ...
        d_knn_dists, d_knn_dists_sort, nullptr, d_sort_idxs,
        batch * k, d_offsets, d_offsets + 1, stream);
...
GPUFuncs::GPUFuncs(
    const float *h_base, int base_n, int dim, int k, int max_degree) : base_n(base_n), dim(dim), k(k), max_degree(max_degree)
{
    cudaDeviceProp prop;
    // 获取 GPU 设备
    ...
    for (auto name : stream_names)
        streams[name] = Stream();
    cudaStream_t &knn_stream = streams["knn"].stream;
    cudaStream_t &upd_stream = streams["upd"].stream;

    // 基础向量数据 GPU 内存分配和数据传输（host->device）
    CUDA_CHECK(cudaMallocAsync(&d_b, base_n * dim * sizeof(float), knn_stream));
    CUDA_CHECK(cudaMemcpyAsync(
        d_b, h_base, base_n * dim * sizeof(float), // 字节数
        cudaMemcpyHostToDevice, knn_stream));
    // 查询向量数据（一个 batch） GPU 内存分配
    CUDA_CHECK(cudaMallocAsync(&d_q, BATCH * dim * sizeof(float), knn_stream));

    CUDA_CHECK(cudaMallocAsync(&d_knn_res, BATCH * k * sizeof(uint32_t), knn_stream));

    CUDA_CHECK(cudaMallocAsync(&d_new_nbr_ids, BATCH * k * max_degree * sizeof(uint32_t), knn_stream));
}
...
GPUFuncs::~GPUFuncs()
{
    ...
    CUDA_CHECK(cudaFreeAsync(d_q, knn_stream));
    CUDA_CHECK(cudaFreeAsync(d_b, knn_stream));
    CUDA_CHECK(cudaFreeAsync(d_knn_res, knn_stream));
