    ...
    : cache_file(cache_file), graph_file(graph_file), k(k), save_thres(10), gpufuncs(gpufuncs)
{
    ...
    if (USE_GRAPH_CACHE)
        LoadGraphCache(
            graph, graph_deg, graph_dist, noniso_num, last_batch_id, total_degree, total_time,
            tres, n);
}

Cache::~Cache() {}

BatchResult Cache::WriteGTCache(
    int *d_knn, const int batch_id, const int batch, const double time_ms)
{
    if (save_future.valid())
    {
        if (save_future.wait_for(std::chrono::seconds(5)) ==
    ...
    if (!USE_GT_CACHE || batch_id < results.size())
        return new_res;
