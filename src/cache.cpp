...
// 构造函数，用于初始化Cache对象
Cache::Cache(const std::string cache_file, const std::string graph_file, GPUFuncs* gpufuncs,
    ...
    : cache_file(cache_file), graph_file(graph_file), k(k), save_thres(10), gpufuncs(gpufuncs) {
    if (USE_GT_CACHE) LoadGTCache();
...
BatchResult Cache::WriteGTCache(int* d_knn, const int batch_id, const int batch,
                                const double time_s) {
    ...
    BatchResult new_res(d_knn, k, batch, time_s);
    if (!USE_GT_CACHE || batch_id < results.size()) return new_res;

    results.push_back(std::move(new_res));
...
void Cache::LoadGTCache() {
    ...
    for (size_t i = 0; i < n; ++i) {
        ...
        if (!file.read(reinterpret_cast<char*>(res.knn.data()), sizeof(int) * k * batch))
            fo.eprint("Read closest_points failed, result size: " +
    ...
    fo.print("Load GT cache finished.");
}
...
void Cache::SaveGTCache() {
    ...
    for (const auto& res : results) {
        ...
        fwrite(res.knn.data(), sizeof(int), k * BATCH, file);
    ...
    if (std::rename(tmp_file.c_str(), cache_file.c_str()) != 0) {
        ...
        fo.print("gt cache saved, total batch num: " + std::to_string(n));
}
...
void Cache::SaveGraphCache(const std::vector<std::vector<int>>& graph,
    ...
    // 序列化时间数据
    const std::string avg_times_str = gpufuncs->avg_time_serialize();
...
void Cache::LoadGraphCache(std::vector<std::vector<int>>& graph, std::vector<int>& graph_deg,
    ...
    gpufuncs->avg_time_deserialize(avg_times_str);
