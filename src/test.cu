...
void GPUFuncs::neibor_aware_test(const std::vector<int>& h_knn_ids, NBD& h_upd_node_data) {
    // const std::string name = "upd";
    // cudaStream_t& stream = streams[name].stream;
    // std::vector<int> cpu_slot_idxs, gpu_slot_idxs;
    // graph->load_data(h_knn_ids, cpu_slot_idxs, gpu_slot_idxs, d_node_data_buffer,
    // h_knn_ids.size(),
    //                  stream);

    // int* d_knn_ids;
    // CUDA_CHECK(cudaMallocAsync(&d_knn_ids, TEST_BATCH * bp.k * sizeof(int), stream));
    // CUDA_CHECK(cudaMemcpyAsync(d_knn_ids, h_knn_ids.data(), TEST_BATCH * bp.k * sizeof(int),
    //                            cudaMemcpyHostToDevice, stream));
    // neibor_aware(d_knn_ids, d_node_data_buffer, TEST_BATCH, bp.k, stream);

    // CUDA_CHECK(cudaMemcpyAsync(&h_upd_node_data, d_node_data_buffer, sizeof(NBD),
...
//             d_b.data().get(), cand_nbrs, res.first, nbr_num, i, TEST_BATCH, cand_size, bp.k,
//             bp);
...
//         knn_idxs, res.first, graph.data().get(), graph_deg.data().get(),
//         graph_dist.data().get(), cand_nbrs, bp.k, TEST_BATCH, bp);
...
//     add_reverse_kernel<<<TEST_BATCH, 32, 0, stream>>>(d_cand_nbrs, nullptr, nullptr,
//     old_nbr_nums,
//                                                       bp);
// #else
...
//                                TEST_BATCH * bp.k * cand_size * sizeof(CN),
//                                cudaMemcpyDeviceToHost, stream));
...
//                                cudaMemcpyDeviceToHost, stream));
//     CUDA_CHECK(cudaMemcpyAsync(h_res_topk_dists, res_topk_dists, TEST_BATCH * bp.k *
//     sizeof(int),
//                                cudaMemcpyDeviceToHost, stream));
