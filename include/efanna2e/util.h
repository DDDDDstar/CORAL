...
static void GenRandom(std::mt19937& rng, int* addr, int size, int N) {
...
inline float* data_align(float* data_ori, size_t point_num, int& dim) {
#ifdef __GNUC__
#ifdef __AVX__
...
template <typename T>
void load_gt_meta(const char* filename, int& points_num, int& dim) {
    ...
    int calc_contained_pts = (int)((fsize - sizeof(int) * 2) / (dim) / sizeof(T));
    std::cout << "load gt from file: " << filename << " points_num: " << points_num
              << " dim: " << dim << std::endl;
    // if (points_num * 2 != calc_contained_pts) {
    //     std::cerr << "filename: " << std::string(filename) << std::endl;
    //     std::cerr << "Data file size wrong! Get points " << calc_contained_pts
    //               << " but should have " << points_num << std::endl;
    //     throw std::runtime_error("Data file size wrong!");
    // }
    in.close();
}
template <typename T>
void load_meta(const char* filename, size_t& points_num, int& dim) {
    std::ifstream in(filename, std::ios::binary);
    if (!in.is_open()) {
    ...
    size_t fsize = (size_t)ss;
    int calc_contained_pts = (int)((fsize - sizeof(int) * 2) / (dim) / sizeof(T));
    std::cout << "load meta from file: " << filename << " points_num: " << num << " dim: " << dim
              << std::endl;
    if (num > calc_contained_pts) {
        std::cerr << "filename: " << std::string(filename) << std::endl;
        std::cerr << "Data file size wrong! Get points " << calc_contained_pts
                  << " but should have " << num << std::endl;
        throw std::runtime_error("Data file size wrong!");
    }
    points_num = static_cast<size_t>(num);
    in.close();
}

template <typename T, typename T2>
void load_gt_data_with_dist(const char* filename, int& points_num, int& dim, T*& data,
                            T2*& res_dists) {
    std::ifstream in(filename, std::ios::binary);
    if (!in.is_open()) {
        std::cout << "open file error" << std::endl;
    ...
    in.seekg(sizeof(int) * 2, std::ios::beg);
    ...
    // cursor position
    std::ios::pos_type ss = in.tellg();
    if ((size_t)ss != points_num * dim * sizeof(T) * 2 + sizeof(int) * 2) {
...
template <typename T>
void load_gt_data(const char* filename, int& points_num, int& dim, T*& data) {
    std::ifstream in(filename, std::ios::binary);
    if (!in.is_open()) {
        std::cout << "open file error" << std::endl;
        exit(-1);
    ...
    in.seekg(sizeof(int) * 2, std::ios::beg);
    data = new T[points_num * dim];
    in.read((char*)(data), points_num * dim * sizeof(T));
    // for (size_t i = 0; i < points_num; i++) {
    // in.seekg(8 + i * (dim + 1) * sizeof(T), std::ios::beg);
    //     in.read((char*)(data + i * dim), dim * sizeof(T));
    // }
    // cursor position
    std::ios::pos_type ss = in.tellg();
    if ((size_t)ss < points_num * dim * sizeof(T) + sizeof(int)) {
        std::cerr << "Read file incompleted!" << std::endl;
        throw std::runtime_error("Data file size wrong!");
    }
    in.close();
...
template <typename T>
void load_data(const char* filename, size_t& points_num, int& dim, T*& data) {
    ...
    if (new_dim == d)
        in.read((char*)(data), pts * new_dim * sizeof(T));
    else
        for (size_t i = 0; i < points_num; i++) {
            // in.seekg(8 + i * (dim + 1) * sizeof(T), std::ios::beg);
    ...
    // cursor position
    std::ios::pos_type ss = in.tellg();
    if ((size_t)ss != pts * d * sizeof(T) + sizeof(int) * 2) {
        std::cerr << "Read file incompleted! filename:" << std::string(filename) << std::endl;
        throw std::runtime_error("Data file size wrong!");
    }
    std::cout << "Finish load data from file: " << filename << " points_num: " << points_num
              << " dim: " << dim << std::endl;
    in.close();
}

template <typename T>
void load_data_host(const char* filename, int points_num, int dim, T*& data) {
    std::cout << "load data from file: " << filename << std::endl;
    std::ifstream in(filename, std::ios::binary);
    if (!in.is_open()) {
        std::cout << "open file error" << std::endl;
...
class TimeMetric {
   public:
    ...
    void print(const std::string& prompt) {
        std::cout << prompt << ": " << elapsed_ << "ms" << std::endl;
    }
