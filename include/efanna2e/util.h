...
namespace efanna2e
{
    ...
    inline float *data_align(float *data_ori, int point_num, int &dim)
    {
#ifdef __GNUC__
#ifdef __AVX__
...
// std::cout << "align to new dim: "<<new_dim << std::endl;
    ...
    template <typename T>
    void load_meta(const char *filename, int &points_num, int &dim)
    {
        std::ifstream in(filename, std::ios::binary);
        if (!in.is_open())
        {
    ...
    void load_gt_data_with_dist(const char *filename, int &points_num, int &dim, T *&data, T2 *&res_dists)
    {
        std::ifstream in(filename, std::ios::binary);
        if (!in.is_open())
        {
    ...
    void load_gt_data(const char *filename, int &points_num, int &dim, T *&data)
    {
        ...
        // cursor position
        std::ios::pos_type ss = in.tellg();
        if ((size_t)ss != points_num * dim * sizeof(T) + sizeof(int) * 2)
        {
            std::cerr << "Read file incompleted!" << std::endl;
            throw std::runtime_error("Data file size wrong!");
        }
    ...
    template <typename T>
    void load_data(const char *filename, int &points_num, int &dim, T *&data)
    {
