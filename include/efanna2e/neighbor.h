...
namespace efanna2e
{
    ...
    struct Neighbor
    {
        ...
        float distance;
        bool flag;
        std::shared_ptr<Neighbor> next; // 所有权关系
        std::weak_ptr<Neighbor> prev;   // 所有权关系

        Neighbor() = default;
        Neighbor(int id, float distance, bool f) : id{id}, distance{distance}, flag(f), next(nullptr) {}
        Neighbor(int id, float distance,
                 std::shared_ptr<Neighbor> next,
                 std::shared_ptr<Neighbor> prev, bool f)
            : id{id}, distance{distance}, flag(f), next(next), prev(prev) {}
    ...
    struct nhood
    {
        ...
        nhood(int l, int s, std::mt19937 &rng, int N)
        {
            M = s;
            nn_new.resize(s * 2);
            GenRandom(rng, &nn_new[0], (int)nn_new.size(), N);
            nn_new.reserve(s * 2);
            pool.reserve(l);
        }
    ...
    static inline int InsertIntoPool(Neighbor *addr, int K, Neighbor nn)
    {
